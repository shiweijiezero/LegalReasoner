...
def generate_issue_prompt(content, jinja2_template,tokenizer, mode="issue"):
    """
    Generate a prompt for the document content.
    """
    prompt = jinja2_template.render(plaintiff_claim=content["plaintiff_claim"],
...
def prepare_data_for_llama_factory(args):
    ...
    # 加载具体模板文件
    jinja2_template = env.get_template('issue_generate.jinja2')
    prompts = [generate_prompt(data, jinja2_template, mode="issue") for data in all_data]
    # 保存数据
    if not os.path.exists("data/sft_data"):
        os.makedirs("data/sft_data")
    with open(f"data/sft_data/sft_issue_generate.json", 'w', encoding='utf-8') as f:
        json.dump(prompts, f, indent=4, ensure_ascii=False)
    return f"sft_data/sft_issue_generate.json"
...
def train_by_llama_factory(init_model, dataset_alias, output_dir, wandb_run_name, per_device_train_batch_size):
    if "llama" in init_model.lower():
        ...
        template_str = "qwen"
    cmd = f"""MKL_THREADING_LAYER=GNU DISABLE_VERSION_CHECK=1 llamafactory-cli train --model_name_or_path {init_model} --stage sft --deepspeed examples/deepspeed/ds_z3_config.json --do_train true --finetuning_type full --dataset {dataset_alias} --template {template_str} --cutoff_len 12800 --overwrite_cache true --preprocessing_num_workers 32 --output_dir {output_dir} --logging_steps 20 --save_steps 5000 --plot_loss true --overwrite_output_dir true --per_device_train_batch_size {per_device_train_batch_size} --gradient_accumulation_steps 1 --learning_rate 1.0e-4 --num_train_epochs 2.0 --lr_scheduler_type cosine --warmup_ratio 0.05 --bf16 true --ddp_timeout 180000000 --report_to wandb --run_name {wandb_run_name}"""
...
def generate_issues(args, model, tokenizer):
    base_path = args.base_path
    input_output_pairs = []
    ...
    for root, dirs, _ in os.walk(base_path):
        if 'EN' in dirs:
            input_dir = os.path.join(root, 'EN')
            relative_path = os.path.relpath(input_dir, base_path)
            output_dir = os.path.join(f'generate_issues_data_{args.model_prefix}', relative_path)
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            input_output_pairs.append((input_dir, output_dir))
    ...
    # 加载具体模板文件
    jinja2_template = env.get_template('issue_generate.jinja2')
    prompts = [generate_issue_prompt(data, jinja2_template,tokenizer, mode="issue") for data in all_data]
    # 生成issues
    ...
    for i, content in enumerate(res_contents):
        all_data[i]["generate_issues"] = content
    # 保存数据到output_files
    for i, output_path in enumerate(output_files):
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(all_data[i], f, indent=4, ensure_ascii=False)
...
def main(args):
    ...
    dataset_alias = "sft_issue_generate"
    ...
    generate_issues(args, model, tokenizer)
